...
Here's what the document covers, structured as a direct answer to each of your questions:

**How the trigger happens** — the NIC writes the frame to RAM via DMA, then fires a hardware interrupt. The ISR (`igb_intr`) immediately disables further interrupts and schedules a **NAPI poll**. The poll runs in softirq context and drains the DMA ring — this is the anti-livelock design.

**How `sk_buff` is born** — `napi_alloc_skb()` allocates the control structure from a slab cache. The actual packet bytes (already in the DMA page) are attached via `skb_add_rx_frag()` — **no copy**, just a pointer and a page reference count bump.

**How pointer reading works** — every layer header macro (`ip_hdr()`, `icmp_hdr()`) is just a cast of `skb->head + offset` into the appropriate struct. `skb_pull()` advances `skb->data` by the header size so the next layer's header is now at `skb->data`. Same bytes in RAM the whole time.

**How protocol dispatch works (twice)** — L2→L3 uses `ptype_base[]` (a hash table keyed by EtherType, registered via `dev_add_pack()`). L3→L4 uses `inet_protos[]` (an array indexed by IP protocol number, registered via `inet_add_protocol()`). Both are just table lookups → function pointer calls.

**Who frees the memory** — the function that *consumes* the `sk_buff` owns the free. `icmp_rcv()` calls `kfree_skb(skb)` on the incoming packet. The outgoing reply is a *new* `sk_buff` — freed by the TX completion ISR via `dev_kfree_skb_irq()` after the NIC finishes DMA-ing it out. The document also covers the `kfree_skb` vs `consume_skb` distinction and why it matters for drop counters.